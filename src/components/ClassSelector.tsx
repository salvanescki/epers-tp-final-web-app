...
import { crearNightBringer } from "../api/api";
import { LoadingOverlay } from "./LoadingOverlay";

// 🔥 Definí type afuera del componente
...
export const ClassSelector = () => {
  ...
  if (!profile || isExpired) {
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-4 p-6 scanlines overflow-hidden">
        ...
        <Link
          to="/"
          className="px-4 py-3 bg-primary text-primary-foreground text-[0.65rem] border-2 border-primary shadow-retro hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
        >
          VOLVER
        </Link>
      </div>
  ...
  const handleStart = async () => {
    ...
    setStarting(true);
    try {
      if (selectedClass === "nightbringer") {
        const nb = await crearNightBringer({ nombre: profile!.name! });

        if (!nb || !nb.id) {
          toast.error("Error creando NightBringer");
          setStarting(false);
          return;
        }

        setNightBringerId(nb.id);
      }
      navigate("/game");
    } catch (err) {
      console.error("Error al crear NightBringer:", err);
      toast.error("No se pudo crear tu NightBringer");
      setStarting(false);
    }
  };
  ...
  return (
    <div className="h-screen flex flex-col items-center justify-center p-4 scanlines overflow-hidden">
      <LoadingOverlay show={starting} label="CARGANDO" />
      ...
      <div className="relative z-10 w-full max-w-md space-y-6">
        <div className="text-center space-y-3">
          <h1 className="text-xl text-primary pixel-title">
            SELECTOR DE CLASE
          </h1>
        ...
        <div className="p-6 bg-card border-2 border-primary shadow-retro space-y-6">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="space-y-1 text-center">
              ...
              <p className="text-primary text-xs truncate max-w-full">
                {profile.name}
              </p>
              <p className="text-[0.55rem] text-muted-foreground break-all">
                {profile.email}
              </p>
            </div>
          </div>
            ...
            <div className="grid gap-3">
              ...
              <button
                ...
                className={`flex items-start gap-3 p-3 border-2 transition-all ${
                  selectedClass === "nightbringer"
                    ? "bg-primary/20 border-primary shadow-retro"
                    : "bg-muted border-border hover:border-primary"
                }`}
              >
                <span className="text-lg">👻</span>
                <div className="text-left">
                  <p className="text-[0.6rem] text-foreground mb-1">
                    NIGHTBRINGER
                  </p>
                  <p className="text-[0.55rem] text-muted-foreground leading-relaxed">
                    Invoca espíritus en el mapa.
                  </p>
                </div>
              </button>
              ...
              <button
                ...
                className={`flex items-start gap-3 p-3 border-2 transition-all ${
                  selectedClass === "lightbringer"
                    ? "bg-accent/20 border-accent shadow-retro"
        ...
        <p className="text-center text-[0.5rem] text-muted-foreground tracking-widest">
